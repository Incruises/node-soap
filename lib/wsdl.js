...
WSDL.prototype.xmlToObject = function(xml) {
  ...
  if (root.Envelope) {
    ...
    if (body.Fault) {
      ...
      if (body['Fault']) {
        if (body['Fault']['detail']) {
          console.log(body['Fault']['detail']['WSCruiseException']);
        }
      }
...
WSDL.prototype.objectToXML = function(obj, name, nsPrefix, nsURI, isFirst, xmlnsAttr, schemaObject, nsContext) {
  ...
  if (Array.isArray(obj)) {
    for (i = 0, n = obj.length; i < n; i++) {
      ...
      if (!correctOuterNsPrefix) {
        if (item && item['_namespace']) {
          correctOuterNsPrefix = item['_namespace'] + ':';

          delete item['_namespace'];
        }
      }
